...
void CBaseContainer::MoveChildren() {
	...
	int x = 0;
	int longest_column_height = 0;//Cannot exceed input::bounds.second
	...
	for(int idx=0;idx<children.size();idx++){
		...
		//Wrap to Next Column
		if(column_height+c->size.second>input::bounds.second){
	...
	auto abs_pos = AbsolutePosition();
	offset.second=std::min(offset.second,input::bounds.second+offset.second-abs_pos.second-longest_column_height);
}
...
void CBaseContainer::PressOn(int child) {
	g_CatLogging.log("PressOn index #%d",child);
	pressed_child = child;
